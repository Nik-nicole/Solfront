...
def prob_viz(res_letras, res_animal, predicted_action, input_frame, colors):
    output_frame = input_frame.copy()
    
    # Mostrar predicción activa
    cv2.putText(output_frame, f"Modo: {'Letras' if predicted_action in actions_letras else 'Animales'}", 
                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    
    # Determinar qué conjunto de resultados mostrar según la predicción
    if predicted_action in actions_letras:
        res = res_letras
        actions = actions_letras
    else:
        res = res_animal
        actions = actions_animal
...
#def predict_and_update(sequence, model, actions):
...
#        sentence = sentence[-5:]
#    
#    return res, predicted_action, confidence
...
predicted_action = "No se detecta ninguna acción"
    ...
    while cap.isOpened():
        ...
        if hand_detected:
            ...
            if len(sequence) == 20:
                ...
                res_letras = model_letras.predict(np.expand_dims(sequence, axis=0))[0]
                res_animal = model_animal.predict(np.expand_dims(sequence, axis=0))[0]
    
                # Obtenemos las mejores predicciones de cada modelo
                max_letras = np.max(res_letras)
                max_animal = np.max(res_animal)
    
                # Determinamos qué modelo tiene mayor confianza
                if max_letras > max_animal and max_letras > threshold:
                    predicted_action = actions_letras[np.argmax(res_letras)]
                    current_mode = "letras"
                elif max_animal > threshold:
                    predicted_action = actions_animal[np.argmax(res_animal)]
                    current_mode = "animal"
                else:
                    predicted_action = "No se detecta ninguna acción"
                
                # Actualización de la oración
                if predicted_action and (not sentence or predicted_action != sentence[-1]):
                    sentence.append(predicted_action)
                    sentence = sentence[-5:]
                    
                image = prob_viz(res_letras, res_animal, predicted_action, image, colors) 
                
        else:
            # Si no se detectan manos, resetear la predicción
            predicted_action = "No se detecta ninguna acción"
        ...
        cv2.putText(image, f"Predición: {predicted_action}", (3,60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1, cv2.LINE_AA)
